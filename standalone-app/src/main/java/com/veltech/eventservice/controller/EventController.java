...
public class EventController {
    ...
    public ResponseEntity<?> reduceTickets(@PathVariable Long id, @RequestParam int slots) {
        Optional<Event> optionalEvent = eventRepository.findById(id);
        if (optionalEvent.isPresent()) {
            Event event = optionalEvent.get();
            if (event.getAvailableTickets() >= slots) {
                event.setAvailableTickets(event.getAvailableTickets() - slots);
                eventRepository.save(event);
                return ResponseEntity.ok("Tickets reduced successfully");
            } else {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Not enough tickets available");
            }
        }
