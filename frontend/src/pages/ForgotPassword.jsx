...
const ForgotPassword = () => {
  ...
  const handleReset = async (e) => {
    e.preventDefault();
    try {
        await axios.post('http://localhost:8081/api/users/reset-password', { email, newPassword });
  ...
  return (
    <div className="app-container page-transition" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '70vh' }}>
      <div className="glass-panel" style={{ width: '100%', maxWidth: '400px' }}>
        <h2 className="gradient-text" style={{ textAlign: 'center' }}>Reset Access</h2>
        
        {msg && <div style={{ padding: '0.8rem', background: 'rgba(255,255,255,0.05)', borderRadius: '0.5rem', marginBottom: '1.5rem', color: 'var(--primary)', textAlign: 'center' }}>{msg}</div>}
